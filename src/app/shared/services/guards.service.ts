...
export class GuardsService {
  private static async validateToken(token: string): Promise<ValidationResponse> {
    try {
      const response = await axios.post(`${GUARDS_SERVICE_URL}/auth/validate-token`, { token });
  ...
  private static async validateRole(token: string, role: string): Promise<ValidationResponse> {
    try {
      const response = await axios.post(`${GUARDS_SERVICE_URL}/auth/validate-role`, { token, role });
      return response.data;
    } catch (error: unknown) {
      if (error instanceof AxiosError) {
        return { valid: false, message: error.response?.data?.message || 'Role validation failed' };
      }
      return { valid: false, message: 'Role validation failed' };
    }
  }

  private static async validateRoute(token: string, route: string): Promise<ValidationResponse> {
    try {
      const response = await axios.post(`${GUARDS_SERVICE_URL}/auth/validate-route`, { token, route });
      return response.data;
    } catch (error: unknown) {
      if (error instanceof AxiosError) {
        return { valid: false, message: error.response?.data?.message || 'Route validation failed' };
      }
      return { valid: false, message: 'Route validation failed' };
    }
  }
